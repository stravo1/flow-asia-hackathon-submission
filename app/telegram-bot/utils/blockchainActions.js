...
const getGasEstimate = async (methodName, fromAddress, ...argsArray) => {
    ...
    const gasEstimate = await contract.methods[methodName](...argsArray).estimateGas({ from: fromAddress });
    const gasPrice = await web3.eth.getGasPrice();
    return { gasEstimate, gasPrice, etherValue: web3.utils.fromWei(gasEstimate * gasPrice, 'ether') };
}
...
const getMintEnabled = async (address) => {
    const web3 = new Web3(process.env.FLOW_RPC_URL);
    const contract = new web3.eth.Contract(FlowHackathonNFT.abi, process.env.FLOW_HACKATHON_NFT_ADDRESS);
    const mintEnabled = await contract.methods.mintingEnabled().call({ from: address });
    return mintEnabled;
};

const setMintPrice = async (address, privateKey, price, gasEstimate, gasPrice) => {
    const web3 = new Web3(process.env.FLOW_RPC_URL);
...
const getMaxSupply = async (address) => {
    const web3 = new Web3(process.env.FLOW_RPC_URL);
    const contract = new web3.eth.Contract(FlowHackathonNFT.abi, process.env.FLOW_HACKATHON_NFT_ADDRESS);
    const maxSupply = await contract.methods.getTotalSupply().call({ from: address });
    return maxSupply;
};
